...
object SeriesEvents {
  ...
  /**
    ...
    */
  def apply(afterAnimate: Option[(api.Series) => Any] = None,
            checkboxClick: Option[(api.Series, SeriesEvents.CheckboxClickEvent) => Boolean] = None,
            click: Option[(api.Series, SeriesEvents.ClickEvent) => Any] = None,
            hide: Option[(api.Series) => Any] = None,
            legendItemClick: Option[(api.Series, JQueryEvent) => Boolean] = None,
            mouseOut: Option[(api.Series, JQueryEvent) => Any] = None,
            mouseOver: Option[(api.Series, JQueryEvent) => Any] = None,
            show: Option[(api.Series) => Any] = None): SeriesEvents = {
    val afterAnimateOuter = afterAnimate.map(ThisFunction.fromFunction1[api.Series, Any]).orUndefined
    val checkboxClickOuter = checkboxClick.map(ThisFunction.fromFunction2[api.Series, SeriesEvents.CheckboxClickEvent, Boolean]).orUndefined
    val clickOuter = click.map(ThisFunction.fromFunction2[api.Series, SeriesEvents.ClickEvent, Any]).orUndefined
    val hideOuter = hide.map(ThisFunction.fromFunction1[api.Series, Any]).orUndefined
    val legendItemClickOuter = legendItemClick.map(ThisFunction.fromFunction2[api.Series, JQueryEvent, Boolean]).orUndefined
    val mouseOutOuter = mouseOut.map(ThisFunction.fromFunction2[api.Series, JQueryEvent, Any]).orUndefined
    val mouseOverOuter = mouseOver.map(ThisFunction.fromFunction2[api.Series, JQueryEvent, Any]).orUndefined
    val showOuter = show.map(ThisFunction.fromFunction1[api.Series, Any]).orUndefined
