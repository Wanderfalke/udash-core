...
object ChartEvents {
  ...
  /**
    ...
    */
  def apply(addSeries: Option[(api.Chart, ChartEvents.AddSeriesEvent) => Any] = None,
            afterPrint: Option[(api.Chart) => Any] = None,
            beforePrint: Option[(api.Chart) => Any] = None,
            click: Option[(api.Chart, ChartEvents.ClickEvent) => Any] = None,
            drilldown: Option[(api.Chart, ChartEvents.DrilldownEvent) => Any] = None,
            drillup: Option[(api.Chart) => Any] = None,
            drillupall: Option[(api.Chart) => Any] = None,
            load: Option[(api.Chart, JQueryEvent) => Any] = None,
            redraw: Option[(api.Chart, JQueryEvent) => Any] = None,
            selection: Option[(api.Chart, ChartEvents.SelectionEvent) => Any] = None): ChartEvents = {
    val addSeriesOuter = addSeries.map(js.ThisFunction.fromFunction2[api.Chart, ChartEvents.AddSeriesEvent, Any]).orUndefined
    val afterPrintOuter = afterPrint.map(js.ThisFunction.fromFunction1[api.Chart, Any]).orUndefined
    val beforePrintOuter = beforePrint.map(js.ThisFunction.fromFunction1[api.Chart, Any]).orUndefined
    val clickOuter = click.map(js.ThisFunction.fromFunction2[api.Chart, ChartEvents.ClickEvent, Any]).orUndefined
    val drilldownOuter = drilldown.map(js.ThisFunction.fromFunction2[api.Chart, ChartEvents.DrilldownEvent, Any]).orUndefined
    val drillupOuter = drillup.map(js.ThisFunction.fromFunction1[api.Chart, Any]).orUndefined
    val drillupallOuter = drillupall.map(js.ThisFunction.fromFunction1[api.Chart, Any]).orUndefined
    val loadOuter = load.map(js.ThisFunction.fromFunction2[api.Chart, JQueryEvent, Any]).orUndefined
    val redrawOuter = redraw.map(js.ThisFunction.fromFunction2[api.Chart, JQueryEvent, Any]).orUndefined
    val selectionOuter = selection.map(js.ThisFunction.fromFunction2[api.Chart, ChartEvents.SelectionEvent, Any]).orUndefined
