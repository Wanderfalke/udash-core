...
object ExportingContextButton {
  ...
  def apply(align: Option[Align] = None,
            ...
            menuItems: Option[Seq[MenuItem]] = None,
            onclick: Option[() => Any] = None,
            ...
            y: Option[Double] = None): ExportingContextButton = {
    val alignOuter = align.map(_.name).orUndefined
    val enabledOuter = enabled.orUndefined
    val heightOuter = height.orUndefined
    val menuItemsOuter = menuItems.map(_.map(_.toJSObject).toJSArray).orUndefined
    val onclickOuter = onclick.map(js.Any.fromFunction0).orUndefined
    val symbolOuter = symbol.orUndefined
