...
object Global {
  ...
  /**
    ...
    * @param useUTC               Whether to use UTC time for axis scaling, tickmark placement and time display in  <code>Highcharts.dateFormat</code>. Advantages of using UTC is that the time displays equally regardless of the user agent's time zone settings. Local time can be used when the data is loaded in real time or when correct Daylight Saving Time transitions are required.
    */
  def apply(date: Option[js.Object] = None,
            VMLRadialGradientURL: Option[String] = None,
            canvasToolsURL: Option[String] = None,
            getTimezoneOffset: Option[(Any) => Int] = None,
            timezoneOffset: Option[Double] = None,
            useUTC: Option[Boolean] = None): Global = {
    val dateOuter = date.orUndefined
    val VMLRadialGradientURLOuter = VMLRadialGradientURL.orUndefined
    val canvasToolsURLOuter = canvasToolsURL.orUndefined
    val getTimezoneOffsetOuter = getTimezoneOffset.map(js.Any.fromFunction1[Any, Int]).orUndefined
    val timezoneOffsetOuter = timezoneOffset.orUndefined
    val useUTCOuter = useUTC.orUndefined
    new Global {
