...
object YAxis {
  ...
  def apply(allowDecimals: Option[Boolean] = None,
            ...
            visible: Option[Boolean] = None): YAxis = {
    ...
    val gridLineInterpolationOuter = gridLineInterpolation.map(_.name).orUndefined
    ...
    val reversedStacksOuter = reversedStacks.orUndefined
    ...
    val stackLabelsOuter = stackLabels.orUndefined
    val startOfWeekOuter = startOfWeek.map(_.id).orUndefined
    val startOnTickOuter = startOnTick.orUndefined
    val stopsOuter = stops.map(_.map(i => js.Array[Double | String | js.Object](i._1, i._2.c)).toJSArray).orUndefined
