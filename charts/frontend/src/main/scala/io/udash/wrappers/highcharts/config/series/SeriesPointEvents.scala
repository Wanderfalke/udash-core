...
class SeriesPointEvents extends js.Object {
  ...
  /**
    ...
    *
    * @example <a href="http://jsfiddle.net/gh/get/jquery/1.7.2/highslide-software/highcharts.com/tree/master/samples/highcharts/plotoptions/pie-point-events-legenditemclick/" target="_blank">Confirm toggle visibility</a>
    */
...
object SeriesPointEvents {
  ...
  /**
    ...
    */
  def apply(click: Option[(Point, JQueryEvent) => Boolean] = None,
            mouseOut: Option[(Point, JQueryEvent) => Boolean] = None,
            mouseOver: Option[(Point, JQueryEvent) => Boolean] = None,
            remove: Option[(Point, JQueryEvent) => Boolean] = None,
            select: Option[(Point, JQueryEvent) => Boolean] = None,
            unselect: Option[(Point, JQueryEvent) => Boolean] = None,
            update: Option[(Point, JQueryEvent) => Boolean] = None): SeriesPointEvents = {
    val clickOuter = click.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val mouseOutOuter = mouseOut.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val mouseOverOuter = mouseOver.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val removeOuter = remove.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val selectOuter = select.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val unselectOuter = unselect.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
    val updateOuter = update.map(ThisFunction.fromFunction2[Point, JQueryEvent, Boolean]).orUndefined
