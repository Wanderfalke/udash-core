...
object Loading {
  ...
  /**
    ...
    */
  def apply(hideDuration: Option[FiniteDuration] = None,
            labelStyle: Option[String] = None,
            showDuration: Option[FiniteDuration] = None,
            style: Option[String] = None): Loading = {

    val hideDurationOuter = hideDuration.map(_.toMillis.toDouble).orUndefined
    val labelStyleOuter = labelStyle.map(stringToStyleObject).orUndefined
    val showDurationOuter = showDuration.map(_.toMillis.toDouble).orUndefined
